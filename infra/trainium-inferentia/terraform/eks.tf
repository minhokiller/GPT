...
module "eks" {
  ...
  aws_auth_users = [
    {
      userarn  = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:user/hugo.alves@animaeducacao.com.br"
      username = "hugo-alves"
  ...
  eks_managed_node_groups = {
    ...
    core_node_group = {
      ...
      launch_template_tags = {
        #SSM tunnel for private Kubernetes API access
        "atun.io/env" = local.name
        "atun.io/version" = "1" 
        "atun.io/host/${replace(module.eks.cluster_endpoint, "https://", "")}" = jsonencode({
          "proto" = "ssm"
          "local" =  8443
          "remote" = 443
        })
